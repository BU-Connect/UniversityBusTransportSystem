...
export const getChatHistory = catchAsync(async (req, res) => {
  ...
  const sessionId = req.headers['x-session-id'] as string || req.cookies?.sessionId || req.query.sessionId;
...
export const getAIResponse = catchAsync(async (req, res) => {
  ...
  const sessionId = req.headers['x-session-id'] as string || req.cookies?.sessionId || req.body.sessionId;
